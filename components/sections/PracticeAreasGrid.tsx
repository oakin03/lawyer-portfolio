  ...
  return (
    <div className="grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-4 [@media(max-height:500px)]:!grid-cols-4 [@media(max-height:500px)]:!gap-3">
      {PRACTICE_AREAS.map((area, index) => {
        const data = tAreas.raw(area.slug) as PracticeAreaTranslation;
        const title = data.title;
        const isClickable = Boolean(data.description?.trim());
        ...
        if (!isClickable) {
          return (
            <div
              ...
              className="relative aspect-square w-full animate-[fade-in-up_0.5s_ease-out_both] overflow-hidden rounded-lg"
            >
              {card}
            </div>
          );
